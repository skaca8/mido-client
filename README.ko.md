...
`mido-client`는 여러 외부 API 채널을 `application.yml`에 선언하는 것만으로 `RestClient`를 자동으로 구성하고 제공합니다. `@Bean` 메서드나 팩토리 클래스 없이, 반복적인 설정 코드 없이 사용할 수 있습니다.
...
dependencies {
    implementation 'com.github.skaca8:mido-client:1.0.2'
}
```
...
> 특정 릴리즈를 사용하려면 `1.0.2`을 원하는 태그 또는 커밋 해시로 변경하세요.
